...
class CWLWar(War.War):
    def __init__(self, tag, state, team_size, preparation_start_time, start_time, end_time, clan, opponent):
        super(CWLWar, self).__init__(state, team_size,
                                     preparation_start_time, start_time, end_time, clan, opponent)
        self.tag = tag
...
def get(war_tag, clan_tag, header):
    ...
    if war_json['state'] == 'notInWar':
        ...
        war_clan = CWLWarClan(clan_status, war_json[clan_status]['tag'], war_json[clan_status]['name'], war_json[clan_status]['clanLevel'],
        ...
        war_opp = CWLWarClan(opp_status, war_json[opp_status]['tag'], war_json[opp_status]['name'], war_json[opp_status]['clanLevel'],
                             war_json[opp_status]['attacks'], war_json[opp_status]['stars'], war_json[opp_status]['destructionPercentage'], opp_members)

    return CWLWar(war_tag, war_json['state'], war_json['teamSize'], war_json['preparationStartTime'], war_json['startTime'], war_json['endTime'], war_clan, war_opp)
